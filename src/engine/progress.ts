import type { EngineRequest, JobJournal, JobLog, JobNode } from "./types.js";
...
export function buildJobNodes(request: EngineRequest): JobNode[] {
...
export function updateProgress(
  ...
  message: string,
  level: JobLog["level"] = status === "failed" ? "error" : status === "succeeded" ? "success" : "info",
): void {
  const node = journal.nodes.find((item) => item.id === nodeId);
  if (!node) throw new Error(`未知执行节点：${nodeId}`);
  node.status = status;
  ...
  if (journal.logs.length > 100) journal.logs.splice(0, journal.logs.length - 100);
}
